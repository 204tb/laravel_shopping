...
<form action="{{route('admin.item.update',$item->id)}}" class="form" method="post">
    ...
    <div>
        ...
        <input class="form-control" type="text" name="code" placeholder={{$item->code}}>
    </div> 

    <div>
    <label for="">{{__('Price')}}</label>
        <input class="form-control" type="text" name="price" placeholder={{$item->price}}>
    </div>

    <label for="">{{__('label.stock')}}</label>
        <input class="form-control" type="text" name="stock" placeholder={{$item->stock}}>
    </div>
